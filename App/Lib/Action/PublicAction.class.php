		...
		//	dump($userlist);
			if($userlist['userswitches'] !=1 || empty($userlist['userswitches'])){
				...
				//dump($userlist);
					...
					if($userlist['department'] == 8 or $userlist['department'] == 9 or $userlist['department'] == 2){
						
						Session('group',2);
					}else{
						Session('group',$userlist['department']);
					}
					Session('department',$userlist['department']);
					Session('rank',$userlist['rank']);
					Session('inherit',$userlist['inherit']);
					Session('userswitches',$userlist['userswitches']);
					Session('permLevel',$userlist['permLevel']);
					Session('xgpassword',$userlist['xgpassword']);
					//dump($_SESSION);
					//exit;
					$this->user_login();
					
					
					if($userlist['xgpassword'] == 0){
						$this->redirect('modpassword');
					}else{
...
public function modpassword(){
	if(empty($_POST)){
		...
		$user=M('user');
			...
			if(empty($_POST['newspassword'])){
				$this->error('请输入密码');
			}
			if($_POST['newspassword'] != $_POST['renewspassword']){
				$this->error('两次输入密码不一致！！请重新输入！');
			}
		//	dump($_POST);
		//	echo Session('uid');
			
		$password = md5($_POST['newspassword']);
		if($user->where('uid ='.Session('uid'))->setField('password',$password)){
		//	echo $password;
		//	exit;
			$user->where('uid ='.Session('uid'))->setField('xgpassword',1);
			$this->redirect('Index/index');
		}else{
			$_SESSION = array();
