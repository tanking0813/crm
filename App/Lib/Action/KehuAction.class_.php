	...
	public function follow(){
		...
		if($permission=='1'){
			if(Session('group')!='2'){//非销售组
				$retrn=$Cus->Field('num,cusname,contact,salemanId,note,description,gd_time,nextnote,uname')->where($data)->group('gd_cid')->order('gd_time')->select();//管理员操作,查全部 
			}else{
